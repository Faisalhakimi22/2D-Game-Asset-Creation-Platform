...
let mockProjectStore: Record<string, any> = {};
let projectIdCounter = 1;

beforeEach(() => {
    projectIdCounter = 1;
    mockProjectStore = {
        'project_123': { ...mockProject },
        'project_456': {
            ...mockProject,
            id: 'project_456',
...
describe('Project Service - Unit Tests', () => {
    ...
    describe('UT-PROJ-004: update project', () => {
        ...
        it('should throw error for non-existent project', async () => {
            await expect(
                ProjectService.update('nonexistent', 'user_123', { title: 'Test' })
            ).rejects.toThrow('Project not found');
        });

        it('should throw error for wrong user', async () => {
            await expect(
                ProjectService.update('project_123', 'wrong_user', { title: 'Test' })
            ).rejects.toThrow('Project not found');
        });
    });
    ...
    describe('UT-PROJ-005: delete project (soft delete)', () => {
        ...
        it('should throw error for non-existent project', async () => {
            await expect(
                ProjectService.delete('nonexistent', 'user_123')
            ).rejects.toThrow();
        });
    });
...
const ProjectService = {
    async create(input: { user_id: string; title: string; type: string; description?: string; color?: string }) {
    ...
    async update(id: string, userId: string, data: Partial<typeof mockProject>) {
        const project = mockProjectStore[id];
