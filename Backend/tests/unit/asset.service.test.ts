...
import { mockAsset, mockProject, mockUser } from '../setup';

// Mock asset data store
let mockAssetStore: Record<string, any> = {};
...
describe('Asset Service - Unit Tests', () => {
    ...
    describe('UT-AST-005: update asset', () => {
        ...
        it('should throw error for non-existent asset', async () => {
            await expect(
                AssetService.update('nonexistent', 'user_123', { name: 'Test' })
            ).rejects.toThrow('Asset not found');
        });

        it('should throw error for wrong user', async () => {
            await expect(
                AssetService.update('asset_123', 'wrong_user', { name: 'Test' })
            ).rejects.toThrow('Asset not found');
        });
    });
    ...
    describe('UT-AST-006: delete asset (soft delete)', () => {
        ...
        it('should throw error for non-existent asset', async () => {
            await expect(
                AssetService.delete('nonexistent', 'user_123')
            ).rejects.toThrow();
        });
    });
...
const AssetService = {
    ...
    async findById(id: string, userId?: string) {
        ...
        if (userId && asset.user_id !== userId) return null;
        return { ...asset };
    },

    async listByProject(projectId: string, filters: AssetFilters = {}) {
    ...
    async listByUser(userId: string, filters: AssetFilters = {}) {
        let assets = Object.values(mockAssetStore)
    ...
    async update(id: string, userId: string, data: Partial<typeof mockAsset>) {
        const asset = mockAssetStore[id];
